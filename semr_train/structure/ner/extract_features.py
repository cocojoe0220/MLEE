...
def get_senstence_label(all_datas):
    ...
    :param all_datas: 所有的数据
    :return: 句子与标签
    """
    sentences = []
    labels = []
    for data in all_datas:
        contexts = data['sentence']['context']
        sentence = ''
        label = []
...
def kfold_transfer_ner_datas_labels(path,datas,kfold=5):
    """
    ...
    sentences, labels = get_senstence_label(datas)
    ...
    for i in range(kfold):
        ...
        one_fold = {'datas':sentences[start:end],'labels':labels[start:end]}
        start = end
...
def save_features(data_path,datas,max_length, kfold = 5):
    ...
    :return:
    """
    folds_datas_labels = kfold_transfer_ner_datas_labels(data_path,datas,kfold)
