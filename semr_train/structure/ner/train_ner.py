...
class ner_datas:
    ...
    def load_ner_h5(self, data_name):
        with h5py.File(data_name, 'r') as f:
            x = f['ner_datas']['features'][()]
            y = f['ner_datas']['labels'][()]
            real_labels = f['ner_datas']['real_labels'][()]
            length_list = f['ner_datas']['lengths'][()]
...
def train_single_ner(model_path, datas, id_2_labels, batch_size=10, epochs=40, learn_rate=0.001, max_length=300,
                     early_stop_num_max=30):
    """

    :param model_path: 模型存储路径
    :param datas: 训练集和测试集
    :param id_2_labels: 标签转换字典
    :param batch_size: 每次训练的数据量
    :param epochs: 训练的迭代此处
    :param learn_rate: 学习率
    :param max_length: 句子最大长度
    :param early_stop_num_max: 当early_stop_num_max个迭代，精度没有提升，训练停止
    :return: 最高精度
    """
    st = time.time()
    test_x = datas['test_x']
...
def train_ner(model_path, datas, cv=False, max_length=300):
    """
    ...
    if cv:
        ...
        for flod in range(kfold):
            ...
            result = train_single_ner(model_path_i, one_datas, id_2_labels)
        ...
        train_single_ner(model_path, one_datas, id_2_labels)
        shutil.rmtree(data_path)
