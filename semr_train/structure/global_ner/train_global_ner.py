...
def train_single_ner(model_path, datas, id_2_labels, batch_size=5, epochs=30, learn_rate=0.001, max_length=300,
                     early_stop_num_max=7):
    """

    :param model_path:模型地址
    :param datas:数据
    :param id_2_labels:数值和标签的映射关系
    :param batch_size:每次训练的数据量
    :param epochs:训练的迭代次数
    :param learn_rate:初始学习率
    :param max_length:输入的最大长度
    :param early_stop_num_max:当early_stop_num_max个迭代，精度没有提升时，训练停止
    :return: 最高精度
    """
    st = time.time()
    test_x = datas['test_x']
    ...
    with tf.Session(config=config, graph=tf.Graph()) as sess:
        ...
        logger.info('train data lengths: ' + str(result['train_example']))
    return result


def train_global_ner(model_path, datas, max_length=300):
    """
