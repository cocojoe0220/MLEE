...
class BertServer:
    def __init__(self, bert_num):
        self.bert_num = bert_num
        self.processes = {}
        for i in range(self.bert_num):
            self.processes[i] = {
                'model': Fast(),
                'status': 0
            }
            self.processes[i]['model'].predict([['1']])

    def predict(self, text):
        is_run_predict = False
        while not is_run_predict:
            for i in range(self.bert_num):
                if self.processes[i]['status'] == 0:
                    self.processes[i]['status'] = 1
                    results, tokens_list = self.processes[i]['model'].predict(text)
                    self.processes[i]['status'] = 0
                    return results, tokens_list
            time.sleep(0.001)


fast_bert = BertServer(10)
